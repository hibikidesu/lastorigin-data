...
function Process()
	...
	-- 2스 봉인일 경우 1스 사용
		if CreatureAI.IsApplyBuff("Effect_MP_Nest_B07_5") == 1 then

			SubProcess2()
			
		else
			
			-- 2스 봉인이 아닌 상황, 파워부스팅 1단계 상황, 70% 확률로 2스 사용
			if CreatureAI.IsApplyBuff("Effect_MP_Nest_B07_33") == 1   and 30 <= CreatureAI.GetRandomValue(100) then			
							
				SubProcess()
				
			else
				-- 2스 봉인이 아닌 상황, 파워부스팅 2단계 상황, 85% 확률로 2스 사용
				if CreatureAI.IsApplyBuff("Effect_MP_Nest_B07_34") == 1 and 15 <= CreatureAI.GetRandomValue(100) then
				
					SubProcess()
					
				else
					-- 2스 봉인이 아닌 상황, 파워부스팅 3단계 상황, 30% 확률로 2스 사용
					if CreatureAI.IsApplyBuff("Effect_MP_Nest_B07_35") == 1 and 70 <= CreatureAI.GetRandomValue(100) then
