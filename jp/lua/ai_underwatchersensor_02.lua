...
function Process()
    ...
    if CreatureAI.GetHPPer() <= 100 and CreatureAI.IsApplyBuff("Effect_MP_UnderWatcherSensor_TU2_4") == 1 then

        -- ACTOR_CLASS가 2인 대상 중 자신과 가장 가까운 열의 대상으로 한다. 
        if 1 == CreatureAI.SetTarget_ActorClass("skill_slot_2", 2) then
            -- 조건에 맞는 대상을 찾은 경우 
            return 
        else  
    ...
    if count > 0  then 
        ...
        if 1 == CreatureAI.SetTarget_ActorClass("skill_slot_1", 2) then 
            -- 조건에 맞는 대상을 찾은 경우 
            return 
        elseif 1 == CreatureAI.SetTarget_ActorClass("skill_slot_1", 0) then 
            -- 조건에 맞는 대상을 찾은 경우 
            return 
        elseif 1 == CreatureAI.SetTarget_ActorClass("skill_slot_1", 1) then -- 적 캐릭터가 두명 이상 배치된 행중 가장 가까운 열의 대상 
            -- 조건에 맞는 대상을 찾은 경우 
            return 
