...
function Process()
	...
	-- 2스 봉인일 경우 1스 사용
		if CreatureAI.IsApplyBuff("Effect_MP_Nest_EW_5") == 1 then
			...
			if CreatureAI.IsApplyBuff("Effect_MP_Nest_EW_33") == 1   and 30 <= CreatureAI.GetRandomValue(100) then			
							
				SubProcess()
				
			else
				-- 2스 봉인이 아닌 상황, 파워부스팅 2단계 상황, 45% 확률로 2스 사용
				if CreatureAI.IsApplyBuff("Effect_MP_Nest_EW_34") == 1 and 55 <= CreatureAI.GetRandomValue(100) then
				
					SubProcess()
					
				else
					-- 2스 봉인이 아닌 상황, 파워부스팅 3단계 상황, 30% 확률로 2스 사용
					if CreatureAI.IsApplyBuff("Effect_MP_Nest_EW_35") == 1 and 70 <= CreatureAI.GetRandomValue(100) then
