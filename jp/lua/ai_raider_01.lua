...
function Process()
    ...
    if CreatureAI.GetHPPer() > 25 and CreatureAI.IsApplyBuff("Effect_MP_Raider_N_5") == 1 then
    ...
    if count > 0  then 
        ...
        if 1 == CreatureAI.SetTarget_ActorClassAndRowInCnt_By_Near("skill_slot_2", 0, 2) then 
            -- 조건에 맞는 대상을 찾은 경우 
            return 
        elseif 1 == CreatureAI.SetTarget_ActorClassAndRowInCnt_By_Near("skill_slot_2", 1, 2) then 
            -- 조건에 맞는 대상을 찾은 경우 
            return 
        elseif 1 == CreatureAI.SetTarget_RowInCnt_By_Near("skill_slot_2", 2) then -- 적 캐릭터가 두명 이상 배치된 행중 가장 가까운 열의 대상 
            -- 조건에 맞는 대상을 찾은 경우 
            return 
        elseif 1 == CreatureAI.SetTarget_Near("skill_slot_2") then -- 가장 가까운 녀석을 공격 
            return 
        end 
    end 
