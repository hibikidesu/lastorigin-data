...
function Process()
	...
	elseif 1 == CreatureAI.IsApplyBuffTarget("Effect_MP_ProgramMobMT_N_1") or 1 == CreatureAI.IsApplyBuffTarget("Effect_MP_ProgramMobMS_N_1") then
		...
		if 1 == CreatureAI.SetTarget_ApplyBuff("skill_slot_1", "Effect_MP_ProgramMobMT_N_1") then
			return
...
-- 디버프에 걸린 적이 없거나 해당 적을 공격할 수 없는 상태일 경우, 스킬1 > 스킬2 > 이동 > 대기 순으로 처리
function SubProcess()
