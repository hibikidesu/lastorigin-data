...
function Process()
    ...
    if CreatureAI.GetHPPer() <= 100 and CreatureAI.IsApplyBuff("Effect_MP_ExecutionerRB_N_5") == 1 then
    ...
    if count > 0  then 
        ...
        elseif 1 == CreatureAI.SetTarget_ActorClass_By_Near("skill_slot_1", 0) then 
            -- 조건에 맞는 대상을 찾은 경우 
            return 
        elseif 1 == CreatureAI.SetTarget_ActorClass("skill_slot_1", 2) then -- 적 캐릭터가 두명 이상 배치된 행중 가장 가까운 열의 대상 
            -- 조건에 맞는 대상을 찾은 경우 
            return 
