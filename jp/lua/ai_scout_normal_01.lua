...
function Process()
	...
	-- HP가 100% 미만 + 다른 아군이 있고, 발동확률이 33% 일때 
	if CreatureAI.GetHPPer() < 70 and CreatureAI.IsLiveOurTeam() == 1 and CreatureAI.GetRandomValue(100) <= 33 then
		-- 스킬 2번 선택 
        ...
        if count == 0 then
			-- 적이 1번 스킬의 사정 거리 바깥에 존재하고, HP가 100% 미만 + 다른 아군이 있으면 스킬 2선택,  자신외의 아군을 타겟 
			if CreatureAI.GetHPPer() < 100 and CreatureAI.IsLiveOurTeam() == 1 then 
				CreatureAI.SetTarget("skill_slot_2", "ourteam")
			else
