...
function Process()
	...
	-- HP가 25% 초과 + 다른 아군이 있고, 발동확률이 33% 일때 
	if CreatureAI.GetHPPer() > 70 and CreatureAI.IsLiveOurTeam() == 1 then
		-- 스킬 2번 선택 
        ...
        if count == 0 then 
			...
			if CreatureAI.GetHPPer() > 10 and CreatureAI.IsLiveOurTeam() == 1 then 
				CreatureAI.SetTarget("skill_slot_2", "ourteam")
			else
